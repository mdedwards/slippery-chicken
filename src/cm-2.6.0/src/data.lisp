...
;;; generated by scheme->cltl from data.scm on 31-Jan-2024 17:13:29
...
(defmacro %interval-sign (n)
  `(if (= (ldb interval-direction-byte ,n) interval-down-flag)
       -1
       1))
        ...
        (dpb (+ 3 (* (- (%interval-type int) 3) -1))
             ...
             (dpb (mod (- 7 span) 7) interval-letters-byte
                  (dpb
                   ...
                   (dpb (mod (- 12 semi) 12) interval-semitones-byte
                        int)))))
      (mod (- 12 int) 12)))
...
(defun encode-interval (interval &optional letters)
  (let ((direction 1)
    ...
    (setf quality
            (position (symbol-name quality) interval-names :test
                      (lambda (a b)
                        (find a b :key #'symbol-name :test
                              #'string-equal))))
    ...
    (labels ((parse-rhythm-token (str)
             ...
             (next-token-position (string lb len)
               (loop with chr
                     ...
                     until (find chr ops :test #'char-equal)
                     finally (return i))))
      (let* ((len (length rhythm))
        ...
        (loop with num = (/ 1
              ...
              for i from 0
              while (< ub len)
              do (setf op (elt rhythm ub)) (setf lb (+ ub 1)) (setf ub
                                                                      (next-token-position
                                                                       rhythm
                                                                       lb
                                                                       len)) (unless
...
(defmethod amplitude
           ((amp symbol)
            &optional (softest *softest*) (loudest *loudest*)
            (power *power*))
  (amplitude
...
(defun rescale-envelope (env &key x-min x-max y-min y-max)
  (let ((minx nil) (miny nil) (maxx nil) (maxy nil))
    (loop for tail on env by #'cddr
          ...
          for y = (cadr tail)
          do (unless minx
               (setf minx x)) (setf maxx x) (if miny
                                                (setf miny
                                                        (min miny y))
                                                (setf miny
                                                        y)) (if maxy
  ...
  (if (< width 2)
      ...
      (progn
       ...
       (loop for i from start to (- end 1)
             ...
             for v = (elt seq i)
             do (setf (elt seq i) (elt seq j)) (setf (elt seq j) v))
       seq)))

(defun ran
       (
        &key (type ':uniform) from (below 1.0) (state *random-state*)
        a b distribution)
  (let ((num nil) (res nil))
    (if (and from below)
        (progn
    ...
    (flet ((betad (a b s)
           ...
           (exp2 (l s)
             (let ((u nil) (v nil) (2*e^-1 (* 2 (exp -1.0))))
               (loop do (setf u (- 1.0 (random 1.0 s))) (setf v
                                                                (*
                                                                 2*e^-1
                                                                 (random
                                                                  1.0
...
(defun histogram (numbers lo hi slots)
  (let ((hist (make-list slots :initial-element 0)) (rang (- hi lo)))
    (loop with i
          for x in numbers
          if (and (>= x lo) (< x hi))
          do (setf i (floor (* slots (/ (- x lo) rang)))) (setf (elt
                                                                 hist
                                                                 i)
                                                                  (+
                                                                   (elt
