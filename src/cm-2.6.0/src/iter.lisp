...
;;; generated by scheme->cltl from loop.scm on 31-Jan-2024 17:13:29
        ...
        ((append collect nconc)
         (let ((head nil) (tail nil))
           (if (not new?)
               ...
               (progn
                (if into
                    (push (make-binding into nil) bind))
                (setf tail (gensym "v"))
                (setf head (gensym "v"))
           ...
           (if (eq oper 'append)
               (progn
                (push `(rplacd ,tail (copy-list ,expr)) loop)
...
(defun parse-while-until (forms clauses ops)
  clauses
  (let ((head forms) (oper (pop forms)) (test nil) (stop '(go :done)))
    (when (null forms)
      (loop-error ops head "Missing '" oper "' expression."))
    (case oper
...
(defun parse-conditional (forms clauses ops)
  (let ((ops (cons '(else) ops))
    ...
    (setf loop
            (list 'if expr (copy-list `(progn ,@(loop-looping then)))
                  nil))
