...
;;; generated by scheme->cltl from midi1.scm on 31-Jan-2024 17:13:30
...
(defun get-meta-msg-type-string (type)
  (let ((res
         (find type +ml-meta-msg-type-strings+ :test #'= :key #'car)))
    (if res
        (cdr res)
        "Unknown Meta Event")))
      ...
      (let ((p (gm-percussion-channel-p channel)))
        ...
        (cond ((= type 12) (setf data1 (gm-patch-name data1)))
              ...
              ((or (= type 10) (= type 8) (= type 9))
               (if p
                   (setf data1 (gm-drum-kit-name data1)))))))
...
(defun %print-sysex-aux (stream data bytes rest indent)
  (let ((offs 0) (toprint bytes) (n 0) (oldn nil) (blank nil))
    (loop while (> toprint 0)
          do (when indent
               (format stream indent)) (format stream
                                               (format-integer offs 6
                                                #\0)) (format stream
                                                              ":") (setf oldn
                                                                           n) (loop for i below 16
                                                                                    while (<
                                                                                           n
                                                                                           bytes)
                                                                                    do (progn
