...
;;; generated by scheme->cltl from midi2.scm on 31-Jan-2024 17:13:30
...
(progn
 (defclass midi-file-stream (midi-stream)
 ...
 (defparameter <midi-file-stream> (find-class 'midi-file-stream))
 (finalize-class <midi-file-stream>)
 (setf (io-class-file-types <midi-file-stream>) '("*.midi" "*.mid"))
...
(defmethod midi-write-message
           ((msg number) (mf midi-file-stream) time data)
  (let ((fp (io-open mf))
    ...
    (cond
     ((< 0 type 15)
      (write-byte
       ...
       fp)
      (when (> size 1) (write-byte (ldb +enc-data-1-byte+ msg) fp))
      (when (> size 2) (write-byte (ldb +enc-data-2-byte+ msg) fp)))
     ...
     ((= type 15)
      (let ((byt (logior (ash type 4) (midimsg-lower-status msg))))
        (write-byte byt fp)
        (case byt
          ((240)
          ...
          ((246) nil)
          (t (error "~s is not a valid system message type." byt)))))
...
(defun midi-file-set-track (mf track)
  (let ((fil (io-open mf)))
    ...
    (loop for p = +midi-file-header-length+ then (+ p 8
                                                    (midi-file-read-track-header
                                                     mf))
          for c from 0
    ...
    (if (not tuning)
        ...
        (let ((tune nil) (num1 nil) (num2 nil) (data nil) (type nil))
          ...
          (cond
           ...
           ((not (null type)) (setq tune (car type))
            (setf num1
                    (if (consp tuning)
                        (pop tuning)
                        0))
...
(defun midi-file-print (file &key (stream t) (track 0))
  (with-open-io (io file :input)
   ...
   (format stream "~%Track ~s, length ~s~%" (midi-file-track io)
...
(defun linux-play-midi-file (file &optional (fork? t))
  (let ((cmd
         (concatenate 'string *linux-midi-file-player* " " file
                      (if fork?
                          " &"
                          ""))))
    (shell cmd)
...
(defparameter *win-midi-file-player*
...
(defun win-play-midi-file (file &rest args)
  (let ((opts
    ...
    (if (probe-file *win-midi-file-player*)
        ...
        (progn
         (warn
          "The MIDI player ~s does not exist. Set the variable *win-midi-file-player* to the pathname (string) of a MIDI player on your machine and try again."
...
(defun osx-play-midi-file (file &optional (fork? t))
  (let ((cmd
         (concatenate 'string *osx-midi-file-player* " " file
                      (if fork?
                          " &"
                          ""))))
    (shell cmd)
