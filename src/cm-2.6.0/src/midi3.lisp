...
;;; generated by scheme->cltl from midi3.scm on 31-Jan-2024 17:13:30
...
(defmacro ensure-microtuning (keyn chan stream)
  `(let ((num nil) (rem nil) (dat nil))
     (cond ((integerp ,keyn) nil)
           ...
           ((floatp ,keyn) (setf dat (midi-stream-tunedata ,stream))
            (if (null dat)
...
(defmethod write-event ((obj midi) (mf midi-file-stream) time)
  (let ((beats time)
    ...
    (cond
     ((integerp ampl)
      (if (= ampl 0)
          ...
          (if (<= 1 ampl 127)
              nil
              (error
               "MIDI: integer amplitude ~s not 0-127 inclusive."
               ampl))))
     ((floatp ampl)
      (if (= ampl 0.0)
          (setf keyn -1)
     ...
     (t
      (error
       "MIDI amplitude ~s is not an integer 0-127 or float 0.0-1.0."
       ampl)))
...
(defmethod write-event
           ((obj standard-object) (mf midi-file-stream) scoretime)
...
(defmethod write-event ((obj midi) (fil clm-stream) scoretime)
  (let ((ins (midi-channel->name (midi-channel obj))))
    (if ins
        (format (io-open fil) "(~a ~s ~s ~s ~s)~%" ins scoretime
                (midi-duration obj) (hertz (midi-keynum obj))
                (midi-amplitude obj)))))
...
(defmethod write-event ((obj midi) (fil clm-audio-stream) scoretime)
  (let ((ins (midi-channel->name (midi-channel obj))))
    (if ins
        (funcall (symbol-function ins) scoretime (midi-duration obj)
                 (hertz (midi-keynum obj)) (midi-amplitude obj)))))

(define-list-struct tc ticks scaler offset)

(defmethod import-events
           ((io midi-file-stream)
            &key (tracks t) seq meta-exclude channel-exclude
            (time-format ':beats) tempo exclude-tracks
            (keynum-format ':keynum) (note-off-stack t))
  (let ((results '())
        (notefn nil)
        (result nil)
        (class nil)
    ...
    (with-open-io (file io :input)
     ...
     (cond ((typep seq <seq>))
           ((or (not seq) (find-class seq))
            ...
            (setf seq nil))
           (t (setf root (format nil "~s" seq)) (setf class <seq>)
            (setf seq nil)))
     ...
     (dolist (track tracks)
       ...
       (cond
        ...
        (t
         (error "track '~s' out of range. Maximum track is ~s." track
                (- num-tracks 1)))))
