...
;;; generated by scheme->cltl from objects.scm on 31-Jan-2024 17:13:29
    ...
    (do ((slots (class-slots class) (cdr slots))
      ...
      (if (slot-boundp obj s)
          (progn
           ...
           (unless (null k)
             (unless
                 (and (eq *print-instance* ':terse)
                      (eq v (slot-definition-initform d)))
               (format port " ~a ~s"
                       (string-downcase (symbol-name s)) v))))))
    (format port ")")
    obj))
...
(defmethod list-subobjects
           ((obj seq) &key start end start-time end-time)
  (let ((subs (subobjects obj)))
    (if (or start-time end-time)
        (progn
         (when (or start end)
           (error
            ":start and :end exclude :start-time and :end-time."))
...
(defun insure-parameters (pars decl supers)
  (flet ((getslotd (slot sups)
           (do ((tail sups (cdr tail))
             ...
             (setf isit
                     (find slot (class-slots (car tail)) :key
