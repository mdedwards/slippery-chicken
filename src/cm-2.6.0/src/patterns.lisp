...
;;; generated by scheme->cltl from patterns.scm on 31-Jan-2024 17:13:30
...
(progn
 (defclass pattern (container)
           ((flags :initarg :flags :initform 0 :accessor
             pattern-flags)
            (data :initarg :of :initarg :data :initarg :notes
             :initarg :keynums :initarg :rhythms :initarg :amplitudes
...
(defun maybeparse (fn val)
  (if (or (not fn) (typep val <pattern>))
      val
      (funcall fn val)))

(defmethod canonicalize-pattern-data
           ((obj pattern) data parser inits)
  inits
  (if parser
      (loop for d in data
            count d into len
        ...
        (let ((flags (pattern-flags obj))
          ...
          (cond
           ((eq state +eop+)
            ...
            (setf (pattern-datum obj) +nad+) (setf state nil))
           (t
          ...
          (if (= (period-count period) 0)
              (progn
               (setf state +eop+)
               (period-reps-set! period (+ 1 (period-reps period))))
              (setf state state))
          (if retfn
              (setf value (funcall retfn value)))
...
(defmethod skip-datum? ((obj pattern))
  (let ((period (pattern-period obj)))
    (if (> (period-omit period) 0)
        (progn
         (period-omit-set! period (- (period-omit period) 1))
         +nad+)
        obj)))
  ...
  (let ((cycl (pattern-data obj)))
    (when (null (cycl-tail cycl))
      (let ((mode (next-1 (palindrome-elide obj)))
        ...
        (cond
         ((eq mode t) (setf long (- (* long 2) 2))
          (setf next (append half (cdr (reverse (cdr half))))))
         ((not mode) (setf long (* long 2))
          (setf next (append half (reverse half))))
         ((member mode '(:first first :start start :left left) :test
                  #'eq)
          (setf long (- (* long 2) 1))
          (setf next (append half (reverse (cdr half)))))
         ((member mode '(:last last :end end :right right) :test
                  #'eq)
          (setf long (- (* long 2) 1))
...
(defmethod next-in-pattern ((obj heap))
  (flet ((shufl (lis len state)
           (loop for i below len
                 ...
                 for v = (elt lis i)
                 do (setf (elt lis i) (elt lis j)) (setf (elt lis j)
                                                           v))
           lis))
    (let ((cyc (pattern-data obj)))
      (if (null (cycl-tail cyc))
...
(defmethod initialize-instance :after ((obj random) &rest args)
  ...
  (let ((pool (pattern-data obj))
    ...
    (loop for item in pool
          ...
          do (progn
              (incf sum weight)
              (random-item-index-set! item sum)) else
          do (setf sum nil)
                    ...
                    (dolist (item pool)
                      (random-item-index-set! item
                       (* (/ (random-item-index item) sum)
                          *random-range*)))))
    (when sum
      (unless *random-range* (setf (random-pattern-range obj) sum))
      (setf (pattern-flags obj)
  ...
  (flet ((parse-random-item (extern)
    ...
    (let ((intern (mapcar #'parse-random-item data)))
      (values intern (length intern)
              (not
               (some (lambda (x) (pattern? (random-item-datum x)))
                     intern))))))
...
(defmethod reset-period ((obj random))
  (let ((reset (call-next-method)) (flags (pattern-flags obj)))
    ...
    (unless (logtest flags +constant-weights+)
      (loop with s = (if (integerp *random-range*)
                         0
                         0.0)
            ...
            do (incf s
                     (next-1
                      (random-item-weight
                       item))) (random-item-index-set! item s)
  ...
  (let* ((pool (pattern-data obj)) (last (cdr pool)))
    (if (and (not (null last))
        ...
        (let ((range (random-pattern-range obj))
              ...
              (next nil))
          (setf next
                  (loop for item = (loop with index = (random range
                                                              state)
                                         for x in choices
                                         when (< index
                                                 (random-item-index
    ...
    (let ((const t))
      ...
      (loop for s in data
            ...
            maximize (length (first p)) into order
                     ...
                     (return (values lis (length data) const)))))))
  ...
  (labels ((select-output (range outputs)
    ...
    (let ((past (markov-pattern-past obj))
      ...
      (loop for i in (pattern-data obj)
            ...
            do (return
                (let ((last nil))
                  (setf item (select-output (second i) (cddr i)))
                  (unless (null past)
                    (if (null (cdr past))
                        (rplaca past item)
                        (progn
                         ...
                         (rplaca past item)
                         (rplacd last past)
                         (setf (markov-pattern-past obj) (cdr past))
                         (rplacd (cdr last) '()))))
                  item))
...
(progn
 (defclass graph (pattern)
           ((selector :initform
             (funcall #'(lambda () #'default-graph-node-select))
...
(defmethod initialize-instance :after ((obj graph) &rest args)
  ...
  (let ((nodes (pattern-data obj)) (last (graph-last obj)))
    ...
    (setf (pattern-data obj)
            (cons (elt nodes (graph-starting-node-index obj))
                  nodes)))
  (values))
...
(defmethod canonicalize-pattern-data ((obj graph) data parser inits)
  ...
  (flet ((parse-graph-item (extern)
    ...
    (let ((intern (mapcar #'parse-graph-item data)))
      (values intern (length intern)
              (not
               (some (lambda (x) (pattern? (graph-node-datum x)))
                     intern))))))
...
(defun markov-select (obj node table lastids)
  ...
  (let ((prob
         (loop for tail on (cdr table) by #'cddr
               when (match-ids (car tail) lastids) return (cadr
                                                           tail))))
    (unless prob
        ...
        (let ((node (find next nodes :key #'graph-node-id)))
          (if node
              (progn
               (rplaca graph node)
               (when last
                 (rplacd last (cons (graph-node-id this) (cdr last)))
...
(progn
 (defclass accumulation (pattern)
           ((indices :initform (funcall #'(lambda () (cons 0 0)))
    ...
    (let ((val (elt (pattern-data obj) (car indices))))
      (if (= (car indices) (cdr indices))
          (progn
           (rplaca indices 0)
           (rplacd indices
                   (mod (+ 1 (cdr indices)) (pattern-length obj))))
...
(defun rotate-items (items start end step width)
  (loop for i from start below
        ...
        do (setf (elt items i) b) (setf (elt items (+ i width)) a))
  items)
...
(defmethod pattern-external-inits ((obj rewrite))
  (flet ((fnc (n)
           (cons 'list
                 (append
                  (list
                   (expand-pattern-value (rewrite-node-datum n)))
    ...
    (append
     (list ':of
           (cons 'list
                 (hash-fold (lambda (k v l) k (cons (fnc v) l)) '()
                  (rewrite-table obj))))
     (call-next-method))))
...
(defmethod initialize-instance :after ((obj rewrite) &rest args)
  ...
  (let ((table (make-hash-table :size 103 :test #'equal))
    ...
    (setf nodes
            (loop for id in (if (listp preset)
                                preset
                                (list preset))
...
(defmethod canonicalize-pattern-data
  ...
  (flet ((parse-rewrite-node (extern)
    ...
    (let ((intern (mapcar #'parse-rewrite-node data)))
      (values intern (length intern)
              (not
               (some (lambda (x) (pattern? (rewrite-node-datum x)))
                     intern))))))
  ...
  (labels ((getnode (id table rule)
    ...
    (loop for rule in rules
          collect (loop with form = rule
                        ...
                        do (setf x (pop form))
                        if (or (eq x '->) (eq x ':->))
                        do (setf left nil) else
                        if left
                        collect x into lh else
                        collect x into rh
                        finally (return
                                 (let ((len (length lh)))
                                   (unless (not left)
                                     (error "Missing -> in rule ~s."
                                            rule))
                                   (cond
                                    ((= len 0)
                                     (error
  ...
  (let ((bits (pattern-flags obj)))
    (append
     ...
     (if (range-to obj)
         ...
         (if (range-downto obj)
             (list
              (if (funcall (range-test obj) 0 0 2)
                  ':above
                  :downto)
...
(defmethod initialize-instance :after ((obj range) &rest args)
  ...
  (let* ((raw (pattern-data obj))
    ...
    (when
        (and (%range-initially? bits) (not (%range-unbounded? bits)))
...
(defmethod canonicalize-pattern-data ((obj range) data parser inits)
  ...
  (let ((from 0)
    ...
    (if (or (pattern? from) (pattern? to) (pattern? downto)
            (pattern? by))
        (setf const? nil))
    (values
     (if const?
...
(progn
 (defclass pval nil ((thunk :initarg :of :accessor pval-thunk)))
...
(defmacro pval (expr) `(make-instance <pval> :of (lambda () ,expr)))
...
(defmethod initialize-instance :after ((obj join) &rest args)
  ...
  (let* ((per (pattern-period obj))
    ...
    (cond
     ((not fmat) (setf fmat (make-list len :initial-element ':eop))
