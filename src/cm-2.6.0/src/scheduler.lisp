...
;;; generated by scheme->cltl from scheduler.scm on 31-Jan-2024 17:13:30
...
(defun pq (&rest args)
  (let* ((q
    ...
    (loop for i from 0
          until (null h)
          do (if (> z (car h))
                 (error "Out of order: ~s ~s ~s" z (car h)
                        (%q-head q))) (setf z (car h)) (format t
                                                               "~s. (~s ~s ~s)"
                                                               i
                                                               (car
                                                                h)
