...
;;; generated by scheme->cltl from spectral.scm on 31-Jan-2024 17:13:29
...
(defun fm-spectrum
  ...
  (let ((data nil) (type nil) (bot nil) (top nil))
    ...
    (when spectrum
      (if minimum
          (setf bot (hertz minimum :hz (eq type ':hertz))))
      (if maximum
          (setf top (hertz maximum :hz (eq type ':hertz)))))
    (setf data
    ...
    (unless neg-freq?
      (do ((neg nil)
        ...
        (let ((side (assoc (first neg) spectrum)))
          (if side
              (rplaca (cdr side) (+ (cadr side) (second neg)))
              (let ((p
                     (position-if #'(lambda (x) (< x pos)) spectrum
